...
def save_activities_to_db(user_id, activities):
    ...
    for ride in activities:
        latlng = ride.get("start_latlng") or []
        ...
        cur.execute(
            ...
            INSERT INTO activities (
                ...
                avg_power, max_power, avg_hr, max_hr, start_lat, start_lng
            )
...
def load_activities_from_db(user_id):
    ...
    cur.execute(
        ...
        SELECT
            ...
            moving_time, kudos, comments, prs, max_speed, avg_speed,
            avg_power, max_power, avg_hr, max_hr, start_lat, start_lng, city, state
        FROM activities
        WHERE user_id = %s
        """,
        (user_id,),
...
def reverse_geocode(lat, lng):
    try:
        response = requests.get(
            ...
            headers={"User-Agent": "RideFind3000/1.0"},
            timeout=10,
        )
...
def background_geocode_activities(user_id):
    global active_geocoding

    if user_id in active_geocoding:
        return

    active_geocoding.add(user_id)
    ...
    try:
        ...
        cur.execute(
            ...
            FROM activities
            WHERE user_id = %s AND start_lat IS NOT NULL AND city IS NULL
            """,
            (user_id,),
        )
        rows = cur.fetchall()
        ...
        print(f"Geocoding {len(rows)} activities for user {user_id}")

        for activity_id, lat, lng in rows:
            city, state = reverse_geocode(lat, lng)
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute(
                "UPDATE activities SET city = %s, state = %s WHERE id = %s",
                (city, state, activity_id),
            )
            conn.commit()
            cur.close()
            conn.close()
        ...
        print(f"Geocoding finished for user {user_id}")

    except Exception as e:
        print(f"Geocoding crashed for user {user_id}: {e}")

    finally:
        active_geocoding.discard(user_id)
...
def background_import_all_activities(user_id, access_token):
    ...
    try:
        ...
        background_geocode_activities(user_id)
...
def activities():
    ...
    if user_id not in active_geocoding:
        threading.Thread(
            target=background_geocode_activities,
            args=(user_id,),
            daemon=True
        ).start()
    ...
    states = sorted({a.get("state") for a in all_activities if a.get("state")})
    ...
    for ride in all_activities:
        ...
        if city_filter and city_filter not in (ride.get("city") or "").lower():
            continue

        if state_filter and ride.get("state") != state_filter:
            continue
    ...
    if sort_by == "date_asc":
        ...
        filtered.sort(key=lambda r: (r.get("city") or "").lower(), reverse=True)
    elif sort_by == "city_asc":
        filtered.sort(key=lambda r: (r.get("city") or "").lower())
    elif sort_by == "state_desc":
        filtered.sort(key=lambda r: (r.get("state") or "").lower(), reverse=True)
    elif sort_by == "state_asc":
        filtered.sort(key=lambda r: (r.get("state") or "").lower())
    ...
    sort_links = {
        ...
        "city": build_sort_link("City", "city_desc", "city_asc"),
        "state": build_sort_link("State", "state_desc", "state_asc"),
    }
